 ...
 */
$modx->regClientCSS($modxboilerplate->config['cssUrl'].'mgr.css');
$modx->regClientStartupScript($modxboilerplate->config['jsUrl'].'mgr/modxboilerplate.js');
$modx->regClientStartupHTMLBlock('<script type="text/javascript">
Ext.onReady(function() {
    modxboilerplate.config = '.$modx->toJSON($modxboilerplate->config).';
    modxboilerplate.config.connector_url = "'.$modxboilerplate->config['connectorUrl'].'";
    modxboilerplate.action = "'.(!empty($_REQUEST['a']) ? $_REQUEST['a'] : 0).'";
});
</script>');
